...
const init = () => {
  ...
  // map.value.on('click', ({latlng, layerPoint, containerPoint, originalEvent}) => {
  //   console.log('latlng:', latlng, ' layerPoint:', layerPoint, ' containerPoint:', containerPoint, ' originalEvent:', originalEvent)
  // })
  addMapBoxTile();
};
const drawBar = ({point1, point2, angle, barHeight, color}) => {
    ...
    L.polygon([point1, point2, point3, point4], {
      ...
      stroke: false,
      fillOpacity: .9
    }).bindTooltip("my tooltip text").openTooltip().addTo(map.value);
...
const draw = (point, color = 'yellow') => {
  L.circle(point, {color: color}).addTo(map.value);
}
onMounted(() => {
  init();
  //  添加图标箭头
  const point1 = { lat: 24.430583796336172, lng: 118.05084228515626 };
  const point2 = { lat: 24.505112714431142, lng: 118.18405151367189 };
  const angle = calcLatLngAngle(point1, point2);
  drawBar({point1: point1, point2: point2, angle: angle, barHeight: 2000, color: 'red'})
  const point3 = calcLatLngByAngleAndDistance({ lng:point2.lng, lat: point2.lat}, angle +90, 1000);
  const point4 = calcLatLngByAngleAndDistance({ lng:point1.lng, lat: point1.lat}, angle +90, 1000);
  drawBar({point1: point3, point2:point4, angle: angle, barHeight: 1000, color: 'green'})
  // const point6 = calcLatLngByAngleAndDistance({ lng:point3.lng, lat: point3.lat}, angle -90, barHeight);
  // addTextToPath();
  // addImageArrow()
  // addHeatMap();
  L.polyline([point1, point2]).addTo(map.value);
  //  temp code start
  //  temp code end
});
</script>
